...
function loginCtrl($conn) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($u === '' || $p === '') {
            ...
            // Then staff
            $staffUser = authStaff($conn, $u, $p);
            if ($staffUser) {
                $_SESSION['user'] = [
                    'id' => $staffUser['id'], 'username' => $staffUser['username'],
                    'name' => $staffUser['name'], 'role' => 'staff'
                ];
                if ($remember) setcookie('remember_user', $u, time() + 86400 * 30, '/');
                else setcookie('remember_user', '', time() - 3600, '/');
...
function adminCtrl($conn) {
    ...
    if ($action === 'edit' && !$editing) {
        $id = intval($_GET['id'] ?? 0);
        $editing = getStaffMember($conn, $id);
    }
...
function staffCtrl($conn) {
    ...
    if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($title === '' || $platform === '' || $quantity === '' || $price === '') {
            $error = 'All fields are required.';
            ...
            if (addGame($conn, $title, $platform, intval($quantity), floatval($price), $staffId)) {
    ...
    if ($action === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($title === '' || $platform === '' || $quantity === '' || $price === '') {
            $error = 'No field can be empty (NULL). All fields are required.';
            $editing = ['id' => $id, 'title' => $title, 'platform' => $platform,
            ...
            if (updateGame($conn, $id, $title, $platform, intval($quantity), floatval($price))) {
